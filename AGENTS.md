...
这是一个以 Git 为内容数据库的静态个人主页。项目、文章、资料、友站和个人设置通过 Pages CMS 编辑并保存到 `content/`，构建时由 `scripts/generate-content.mjs` 使用 Zod 校验，再生成忽略提交的 `app/generated/content.ts`。页面、组件和元数据路由位于 `app/`；测试位于 `tests/`；图片等静态资源位于 `public/`。受密码保护的长篇是例外：明文只保存在被忽略的 `private-content/fiction/`，仓库仅提交 `public/protected/fiction/` 密文。
...
截至 2026-09-16，已快进同步远端内容提交 `d407734`，保留新增随笔。Pages CMS 的项目、校园手记、随笔、资料库和友站统一使用 `slug` 生成 JSON 文件名，中文标题不再导致空文件名或扩展名错误。新增的 `/friends` 友站页已加入侧栏导航和 sitemap；友站条目保存在 `content/friends/`，可自助配置站名、个人主页地址、简介、头像、标签、置顶和发布开关，网址只允许 HTTP/HTTPS，未发布任何友站时显示空状态。首页下方介绍、项目页、校园手记页、随笔页、资料库页和友站页的标题/说明均可在“个人空间设置”中编辑。受保护长篇的侧栏显式入口现已通过 `app/lib/features.ts` 中的 `showFictionNavigation` 开关关闭，但 `/fiction` 直达页面、浏览器端密码解锁、按需解密阅读、加密脚本和密文均保留；将开关设为 `true` 即可恢复入口。密码格式按需求限制为 4 位数字，但前端输入框不显示格式提示、不强制数字键盘或输入长度。长篇草稿使用“每篇一个 JSON 元数据 + 一个 Markdown 正文”的本地格式，JSON 通过 `bodyFile` 指向同目录 `.md` 文件。当前已生成 9 篇长篇密文，明文仍仅保存在已忽略的 `private-content/fiction/`。四位密码安全性较低，只适合一般访问门槛。
